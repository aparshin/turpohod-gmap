...
function Main () {
  if (GBrowserIsCompatible()) {
    ...
    div.appendChild(document.createTextNode("Your brouser does not support GMap. Sorry."));
  }
}
...
function ShowMapA () {
  ...
  if (data.name != null) {
    if (isFrame)
      parent.document.title = data.name;
    else
  ...
  if (isFrame) {
    ...
    var h2 = docTrack.createElement("h2");
    if (xmldesc != null)
      xml2html(xmldesc, docTrack, h2);
    else
      h2.appendChild(docTrack.createTextNode(name));
    docTrack.getElementById("body").appendChild(h2);
  }
...
function WaypointsCheckBox () {
  if (document.getElementById("waypointscheckbox").checked) {
    for (var i = 0; i < data.waypoints.length; i++)
      map.addOverlay(data.waypoints[i]);
    ...
    for (var i = 0; i < data.waypoints.length; i++)
      map.removeOverlay(data.waypoints[i]);
}
...
function addBigIcon (waypoint) {
  BigIcon.gMarker = new GMarker(waypoint.getPoint(), {title:waypoint.name, icon:BigIcon});
  map.addOverlay(BigIcon.gMarker);
  BigIcon.gMarker.waypoint = waypoint;
  GEvent.addListener(BigIcon.gMarker, "click", function() { MouseClick(this.waypoint); if (this.waypoint.aElement) this.waypoint.aElement.scrollIntoView();});
  var b = document.createElement("b");
...
function MouseClick (waypoint) {
  ...
  //if (map.getZoom() == 14)
    map.panTo(waypoint.getPoint());
  //else
  //  map.setCenter(waypoint.getPoint(), 14);
}

function MouseOut (waypoint) {
  removeBigIcon();
...
function MouseOver (waypoint) {
  removeBigIcon();
  addBigIcon(waypoint);
  GEvent.addListener(BigIcon.gMarker, "mouseout", function() { MouseOut(this.waypoint); });
}

BigIcon = new GIcon(G_DEFAULT_ICON);
BigIcon.image = "http://turpohod.narod.ru/treks/gmap/icons/marker2.png";
